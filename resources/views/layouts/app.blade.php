...
<body class="antialiased bg-beige-lighter min-h-screen flex flex-col">
    ...
    {{-- Sticky Header with marquee (non-sticky) and nav (sticky) --}}
    <header>
        {{-- Marquee outside sticky nav (not sticky) --}}
        @include('layouts.marquee') {{-- Improved: Ensured inclusion for loading the marquee; partial must exist. --}}

        {{-- Sticky nav only --}}
        <nav class="sticky top-0 z-50 bg-white border-b border-gold shadow-md"> {{-- Improved: Maintained white background with explicit bg-white; enhanced shadow for better visual separation. --}}
            @include('layouts.navigation') {{-- Your nav menu partial without marquee --}}
        </nav>
    </header>
